    ...
    for ri in range(len(radii)):
        ...
        for chunk in chunks:
            ...
            if ri!=0:
                ...
                rpairs += np.sum((len(item) for item in rpairs2)) - np.sum((len(item) for item in rpairs1))
                rindices = [int(j) for j in np.hstack([list(set(j2)-set(j1)) for j1,j2 in zip(rpairs2, rpairs2)])]                
